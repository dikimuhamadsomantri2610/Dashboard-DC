...
export const presensiApi = axios.create({
    baseURL: '/api'
});

export async function checkNik(nik: string) {
    return presensiApi.get(`/presensi/check-nik/${nik}`);
}

export async function getServerTime(): Promise<string> {
    const res = await presensiApi.get('/presensi/time');
...
export async function submitPresensi(
    ...
    return presensiApi.post('/presensi', { nik, jenis, waktu, ...extraData });
}
