...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "flex flex-col gap-5 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-card py-5 text-card-foreground shadow-sm dark:shadow-none",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("leading-none font-semibold tracking-tight text-[0.9375rem]", className)}
      {...props}
    />
  )
}
