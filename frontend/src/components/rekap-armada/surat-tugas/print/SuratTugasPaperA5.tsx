                ...
                <div className="grid grid-cols-2 gap-x-12 px-2 text-[11px] mb-4">
                    <div className="space-y-1">
                        ...
                        <div className="grid grid-cols-[100px_10px_auto] border-b border-dotted border-black pb-0.5">
                            ...
                            <span className="font-semibold uppercase truncate">{selectedGroupForPrint.nama_driver}</span>
                        </div>
                        <div className="grid grid-cols-[100px_10px_auto] border-b border-dotted border-black pb-0.5">
                            <span>NO ARMADA</span><span>:</span>
                            <span className="font-semibold uppercase truncate">{selectedGroupForPrint.no_armada}</span>
                        </div>
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="grid grid-cols-[100px_10px_auto] border-b border-dotted border-black pb-0.5">
                            ...
                            <span className="font-semibold uppercase truncate">{selectedGroupForPrint.items[0]?.number_seal || ''}</span>
                        </div>
                        {selectedGroupForPrint.items.slice(0, 3).map((item, idx) => (
                            <div key={`load-${idx}`} className="grid grid-cols-[100px_10px_auto] border-b border-dotted border-black pb-0.5">
                                ...
                                <span className="font-semibold uppercase truncate">{item.load_number}</span>
                            </div>
                        ))}
                    ...
                    <tbody>
                        {selectedGroupForPrint.items.map((item, idx) => (
                            <tr key={item.id}>
                                ...
                                <td className="border border-black py-[3px] px-0.5">{item.inisial_toko}</td>
                                ...
                                <td className="border border-black py-[3px] px-0.5">{item.jumlah_container}</td>
                                <td className="border border-black py-[3px] px-0.5">{item.jumlah_koli}</td>
                ...
                <div className="relative flex flex-col mt-2 text-[10px] font-bold">
                    <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', alignItems: 'start', gap: '8px' }}>
                            ...
                            {['ADM. DISTRIBUSI', 'DRIVER', 'SECURITY'].map((title, i) => (
                                <div key={i} className={`flex flex-col border-2 border-black h-[75px] ${i === 0 ? 'w-[120px]' : 'w-[110px]'}`}>
                                    ...
                                    <div className="border-t-2 border-black py-0.5 text-center bg-gray-200/50">
                                        <span className={`uppercase text-[9px] wrap-break-word px-1 ${i < 2 ? 'line-clamp-1' : ''}`} title={i === 0 ? selectedGroupForPrint.admin : i === 1 ? selectedGroupForPrint.nama_driver : ''}>
                                            {i === 0 ? selectedGroupForPrint.admin : i === 1 ? selectedGroupForPrint.nama_driver : '\u00A0'}
                                        </span>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="flex gap-1 flex-wrap justify-center items-center">
                            {selectedGroupForPrint.items.map((item) => (
                                <BarcodeBox key={item.id} value={generateKodeGembok(item.kodeGembok, item.site) || ''} inisial={item.inisial_toko} />
                            ))}
                        </div>
