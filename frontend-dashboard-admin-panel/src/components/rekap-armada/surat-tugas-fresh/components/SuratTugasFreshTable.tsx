...
interface SuratTugasFreshTableProps {
  isLoading: boolean;
  currentData: GroupedSuratTugasFresh[];
  groupedDataFiltered: GroupedSuratTugasFresh[];
  currentPage: number;
  perPage: number;
  totalPages: number;
  setCurrentPage: (val: number | ((prev: number) => number)) => void;
  confirmDeleteGroup: (group: GroupedSuratTugasFresh) => void;
  setGroupToConfirmPrint: (data: {
    group: GroupedSuratTugasFresh;
    index: number;
  }) => void;
}

export default function SuratTugasFreshTable({
  isLoading,
  currentData,
  groupedDataFiltered,
  currentPage,
  ...
  const handleEdit = (group: GroupedSuratTugasFresh) => {
    router.push(
      `/rekap-armada/surat-tugas-fresh/add?groupId=${encodeURIComponent(group.groupId)}`,
    );
  };
              ...
              currentData.map((group) => {
                ...
                return (
                  <TableRow key={group.groupId}>
                    ...
                    <TableCell className="align-middle font-semibold text-zinc-700 dark:text-zinc-300">
                      {group.dc}
                    </TableCell>
                    <TableCell className="align-middle">
